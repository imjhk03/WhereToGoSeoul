 ...
 */
public class DetailTwoActivity_ENG extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String strUrl1 = "http://api.visitkorea.or.kr/openapi/service/rest/EngService/detailCommon?ServiceKey=834oAdPjoTtX%2FCihn6VDwM6KuVD3qhnZxf%2F%2F9vJ3emazbqrFpz1CV%2BULbzJUOhr7c797esiz8MaaeHQoq%2BSCbA%3D%3D&contentId=" + contentid + "&contentTypeId=82&defaultYN=Y&mapImageYN=Y&firstImageYN=Y&areacodeYN=Y&catcodeYN=Y&addrinfoYN=Y&mapinfoYN=Y&overviewYN=Y&transGuideYN=Y&MobileOS=AND&MobileApp=WhereToGoSeoul";
        downTask1.execute(strUrl1);

        DownloadWebpageTask downTask = new DownloadWebpageTask();
        //System.out.println("contentid :"+contentid);
        String strUrl = "http://api.visitkorea.or.kr/openapi/service/rest/EngService/detailIntro?ServiceKey=834oAdPjoTtX%2FCihn6VDwM6KuVD3qhnZxf%2F%2F9vJ3emazbqrFpz1CV%2BULbzJUOhr7c797esiz8MaaeHQoq%2BSCbA%3D%3D&contentId=" + contentid + "&contentTypeId=82&introYN=Y&MobileOS=AND&MobileApp=WhereToGoSeoul";
        downTask.execute(strUrl);

        button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {//버튼을 누르면 실행됨
                Intent intent1 = new Intent(DetailTwoActivity_ENG.this, ShowMap.class);//인텐트로 보내지는곳의 클래스와,보낼곳의 위치 DetailOneActivity->ShowMapOne
    ...
    class DownloadWebpageTask1 extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String result) {
            ...
            try {
                ...
                while (eventType != XmlPullParser.END_DOCUMENT) {
                    if (eventType == XmlPullParser.START_DOCUMENT) {
                        ...
                        if (bSet || bSet2 || bSet3 || bSet4 || bSet5 || bSet6 || bSet7 || bSet8 || bSet9) {
                            ...
                            if (bSet8) {
                                //title = xpp.getText();
                                int start = xpp.getText().indexOf("(");
                                int end = xpp.getText().length();
                                String substr = xpp.getText().substring(start, end);
                                title = xpp.getText().replace(substr, "");
                                bSet8 = false;
                            }
                            if (bSet9) {
                                zipcode = xpp.getText();
                ...
                if (firstimage == null) {
                    Glide.with(DetailTwoActivity_ENG.this)
                ...
                if (zipcode == null) {
                    ...
                    textView_zipcode.setText("- Zipcode : " + zipcode);
                }
                if (addr2 == null) {
                    textView_addr1.setText("- Address : " + addr1);
                } else {
                    textView_addr1.setText("- Address : " + addr1 + " " + addr2);
                }
                if (tel == null) {
                    textView_tel.setText("- Tel : N/A");
